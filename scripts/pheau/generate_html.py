"""
Generates index.html for DSM pH (pHeau) — Oise.
Fixed 7-class pH classification, dark map, collapsible stats sidebar.
"""
...
FO_BASE    = os.path.join(SCRIPT_DIR, "..", "Final_outputs", "Final_outputs", "pHeau")
MASQUE_SHP = os.path.join(SCRIPT_DIR, "..", "data", "masque", "Departements.shp")
OUT_HTML   = os.path.join(SCRIPT_DIR, "index.html")
MAX_PX     = 750

# ── Depth definitions ─────────────────────────────────────────────
# Note: folder "60 - 100 cm" has spaces around the dash
...
# Actual folder names on disk (60-100 has spaces)
DEPTH_FOLDER = {"0-5":"0-5 cm","5-15":"5-15 cm","15-30":"15-30 cm",
                "30-60":"30-60 cm","60-100":"60 - 100 cm","100-200":"100-200 cm"}

# ── Fixed pH classification (7 classes, gradient colormap) ────────
PH_LABELS = ['≤ 4', '4 – 5', '5 – 6', '6 – 7', '7 – 8', '8 – 9', '≥ 9']

# Continuous diverging gradient: red (acidic) → orange → yellow → green → blue (alkaline)
CMAP_PH = plt.get_cmap('Spectral_r')
NORM_PH  = mcolors.Normalize(vmin=4, vmax=9, clip=True)

# Sample legend swatch colors from the gradient at each class midpoint
_mids = [4.0, 4.5, 5.5, 6.5, 7.5, 8.5, 9.0]
def _ph_hex(v):
    return mcolors.to_hex(CMAP_PH(NORM_PH(v))[:3])

FIXED_CLASSES = [
    {"min": -999, "max": 4,    "color": _ph_hex(_mids[0]), "label": PH_LABELS[0]},
    {"min": 4,    "max": 5,    "color": _ph_hex(_mids[1]), "label": PH_LABELS[1]},
    {"min": 5,    "max": 6,    "color": _ph_hex(_mids[2]), "label": PH_LABELS[2]},
    {"min": 6,    "max": 7,    "color": _ph_hex(_mids[3]), "label": PH_LABELS[3]},
    {"min": 7,    "max": 8,    "color": _ph_hex(_mids[4]), "label": PH_LABELS[4]},
    {"min": 8,    "max": 9,    "color": _ph_hex(_mids[5]), "label": PH_LABELS[5]},
    {"min": 9,    "max": 9999, "color": _ph_hex(_mids[6]), "label": PH_LABELS[6]},
]
...
for depth in DEPTHS:
    ...
    # ── RF Mean map ───────────────────────────────────────────────
    tif_path = os.path.join(depth_dir, f"carte_RF_moyenne_pH_eau_{suffix}.tif")
    if os.path.exists(tif_path):
        ...
        # Apply continuous gradient pH colormap
        rgba = CMAP_PH(NORM_PH(np.where(np.isnan(data), 0, data)))
    ...
    # ── Sample points ─────────────────────────────────────────────
    shp_path = os.path.join(depth_dir, f"points_apprentissage_pHeau_{suffix}.shp")
    ...
    imp_path = os.path.join(depth_dir,
                            f"importance_MDI_50iter_redistrib_pH_eau_{suffix}.xlsx")
    ...
    # ── Validation metrics ────────────────────────────────────────
    met_path = os.path.join(depth_dir, f"metriques_iterations_pH_eau_{suffix}.xlsx")
    if os.path.exists(met_path):
        df_met = pd.read_excel(met_path)
        entry["met"]["r2"]   = df_met["r2"].dropna().round(4).tolist()
        entry["met"]["rmse"] = df_met["rmse"].dropna().round(3).tolist()
...
#legend-panel{{
  ...
  padding:13px 16px 11px;min-width:175px;max-width:210px;
...
<div id="header">
  <div id="header-left">
    <h1>Digital Soil Mapping — Soil pH (pHeau)</h1>
...
<!-- Legend -->
<div id="legend-panel">
  <div class="leg-title">Soil pH</div>
  ...
  <div class="leg-pts" id="raster-toggle" onclick="toggleRaster()">
    ...
    <span class="pt-label" id="raster-label">pH map</span>
...
(function buildStaticLegend() {{
  ...
  // raster icon: first → last class gradient
...
function showDepth(d, btn) {{
  ...
  document.getElementById('raster-label').textContent = `pH map (${{depthLabels[d]}} cm)`;
  ...
  if (info.pts && info.pts.features.length > 0) {{
    ptLayer = L.geoJSON(info.pts, {{
      ...
      onEachFeature:(f,layer) => {{
        ...
        layer.bindTooltip(
          v !== null ? `<b>pH:</b> ${{v.toFixed(2)}}` : '<i>No value</i>',
...
function renderCharts(d) {{
  ...
  const maxRmse  = Math.max(...rmses).toFixed(2);
  const minRmse  = Math.min(...rmses).toFixed(2);
  const meanRmse = (rmses.reduce((a,b)=>a+b,0)/rmses.length).toFixed(2);
  ...
  Plotly.react(metEl,
    ...
    {{
      ...
      xaxis2:{{domain:[0.57,1],title:{{text:'RMSE (pH)',font:{{color:'#fdba74',size:11}}}},
        tickfont:{{size:9,color:'rgba(255,255,255,0.42)'}},color:'rgba(255,255,255,0.22)',gridcolor:'rgba(0,0,0,0)'}},
      ...
      annotations:[
        ...
        {{xref:'paper',yref:'paper',x:0.785,y:1.01,
          text:`Max: ${{maxRmse}}<br>Mean: ${{meanRmse}}<br>Min: ${{minRmse}} pH`,
  ...
  if (rv.length === 0) {{
    ...
    Plotly.react(rastEl,
      ...
      {{
        ...
        paper_bgcolor:'rgba(0,0,0,0)', plot_bgcolor:'rgba(255,255,255,0.015)',
        xaxis:{{
          title:{{text:'pH',font:{{color:'rgba(255,255,255,0.36)',size:10}}}},
          color:'rgba(255,255,255,0.32)',gridcolor:'rgba(255,255,255,0.06)',
        ...
        annotations:[{{
          ...
          text:`Max: ${{maxV}} · Mean: ${{meanV}} · Min: ${{minV}}`,
