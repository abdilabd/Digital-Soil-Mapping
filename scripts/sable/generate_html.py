"""
Generates index.html for DSM Sand content (Sable_vc) — Oise.
Fixed 4-class sand classification, dark map, collapsible stats sidebar.
"""
...
FO_BASE    = os.path.join(SCRIPT_DIR, "..", "Final_outputs", "Final_outputs", "Sable_vc")
...
# Folder names on disk match the depth keys directly (no "cm" suffix)
DEPTH_FOLDER = {"0-5":"0-5","5-15":"5-15","15-30":"15-30",
                "30-60":"30-60","60-100":"60-100","100-200":"100-200"}

# ── Fixed sand classification (4 classes, gradient colormap) ──────
SAND_LABELS = ['≤ 400', '400 – 600', '600 – 800', '≥ 800']

# Sequential gradient: yellow-green-blue (low sand → high sand)
CMAP_SAND = plt.get_cmap('YlGnBu')
NORM_SAND  = mcolors.Normalize(vmin=0, vmax=800, clip=True)

# Sample legend swatch colors at each class midpoint
_mids = [200.0, 500.0, 700.0, 800.0]
def _sand_hex(v):
    return mcolors.to_hex(CMAP_SAND(NORM_SAND(v))[:3])

FIXED_CLASSES = [
    {"min": -999, "max": 400,  "color": _sand_hex(_mids[0]), "label": SAND_LABELS[0]},
    {"min": 400,  "max": 600,  "color": _sand_hex(_mids[1]), "label": SAND_LABELS[1]},
    {"min": 600,  "max": 800,  "color": _sand_hex(_mids[2]), "label": SAND_LABELS[2]},
    {"min": 800,  "max": 9999, "color": _sand_hex(_mids[3]), "label": SAND_LABELS[3]},
]
...
for depth in DEPTHS:
    ...
    # ── RF Mean map ───────────────────────────────────────────────
    tif_path = os.path.join(depth_dir, f"carte_RF_moyenne_sable_g_kg_{suffix}.tif")
    if os.path.exists(tif_path):
        ...
        # Apply continuous gradient sand colormap
        rgba = CMAP_SAND(NORM_SAND(np.where(np.isnan(data), 0, data)))
    ...
    # ── Sample points ─────────────────────────────────────────────
    shp_path = os.path.join(depth_dir, f"points_apprentissage_sable_nv_g_kg_{suffix}.shp")
    ...
    imp_path = os.path.join(depth_dir,
                            f"importance_MDI_50iter_redistrib_sable_g_kg_{suffix}.xlsx")
    ...
    # ── Validation metrics ─────────────────────────────────────────
    met_path = os.path.join(depth_dir, f"metriques_iterations_sable_g_kg_{suffix}.xlsx")
...
<title>DSM — Sand Content · Oise</title>
...
<div id="header">
  <div id="header-left">
    <h1>Digital Soil Mapping — Sand Content</h1>
...
<!-- Legend -->
<div id="legend-panel">
  <div class="leg-title">Sand content (g/kg)</div>
  ...
  <div class="leg-pts" id="raster-toggle" onclick="toggleRaster()">
    ...
    <span class="pt-label" id="raster-label">Sand map</span>
...
function showDepth(d, btn) {{
  ...
  document.getElementById('raster-label').textContent = `Sand map (${{depthLabels[d]}} cm)`;
  ...
  if (info.pts && info.pts.features.length > 0) {{
    ptLayer = L.geoJSON(info.pts, {{
      ...
      onEachFeature:(f,layer) => {{
        ...
        layer.bindTooltip(
          v !== null ? `<b>Sand:</b> ${{v.toFixed(1)}} g/kg` : '<i>No value</i>',
  ...
  if (rv.length === 0) {{
    ...
    Plotly.react(rastEl,
      ...
      {{
        ...
        paper_bgcolor:'rgba(0,0,0,0)', plot_bgcolor:'rgba(255,255,255,0.015)',
        xaxis:{{
          title:{{text:'Sand (g/kg)',font:{{color:'rgba(255,255,255,0.36)',size:10}}}},
          color:'rgba(255,255,255,0.32)',gridcolor:'rgba(255,255,255,0.06)',
