"""
Generates index.html for DSM Clay (Argile) — Oise.
Fixes: points clipped to study area, discrete legend matching map colors,
       100-200 cm tab (SHP-only), everything in English.
"""
...
FO_BASE    = os.path.join(SCRIPT_DIR, "..", "Final_outputs", "Final_outputs", "Argile")
...
def hex_color(norm_val):
    r,g,b,_ = cmap(float(norm_val))
    return mcolors.to_hex((r,g,b))

def make_classes(vmin, vmax, n=N_CLASSES):
    """Equal-interval discrete classes with matching colormap hex colors."""
    step = (vmax - vmin) / n
    classes = []
    for i in range(n):
        lo  = vmin + i * step
        hi  = vmin + (i+1) * step
        mid = (i + 0.5) / n          # normalised midpoint → colormap
        classes.append({
            "min":   round(lo, 1),
            "max":   round(hi, 1),
            "color": hex_color(mid),
            "label": f"{lo:.0f}–{hi:.0f} g/kg",
        })
    return classes
...
def shp_to_geojson(shp_path):
    """Read SHP, reproject to WGS84, clip to Oise, return GeoJSON dict."""
    ...
    for _, row in gdf.iterrows():
        ...
        features.append({
            ...
            "properties": {"value": round(v,1) if v else None},
        })
    return {"type":"FeatureCollection","features":features}, len(features)

# ── Global vmin/vmax across all modellable depths ────────────────
print("Computing global value range across all depths...")
_g_vmins, _g_vmaxs = [], []
for _d in [d for d in DEPTHS if d != "100-200"]:
    _suf  = DEPTH_SUFFIX[_d]
    _fld  = f"argile_g_kg_{_suf}"
    _tif  = os.path.join(DATA_DIR, _d, f"carte_RF_moyenne_{_fld}.tif")
    if os.path.exists(_tif):
        with rasterio.open(_tif) as _src:
            _arr = _src.read(1).astype("float32")
            _arr = np.where(_arr == _src.nodata, np.nan, _arr)
        _g_vmins.append(float(np.nanpercentile(_arr, 5)))
        _g_vmaxs.append(float(np.nanpercentile(_arr, 95)))
GLOBAL_VMIN    = min(_g_vmins) if _g_vmins else 0.0
GLOBAL_VMAX    = max(_g_vmaxs) if _g_vmaxs else 1.0
GLOBAL_CLASSES = make_classes(GLOBAL_VMIN, GLOBAL_VMAX)
print(f"  Global range : {GLOBAL_VMIN:.1f} – {GLOBAL_VMAX:.1f} g/kg  ({N_CLASSES} classes)")
...
for depth in DEPTHS:
    ...
    field    = f"argile_g_kg_{suffix}"
    field_nv = f"argile_nv_g_kg_{suffix}"
    depth_dir = os.path.join(DATA_DIR, depth)        # may not exist for 100-200
    fo_depth  = os.path.join(FO_BASE, depth)
    print(f"\nDepth {depth} cm ...")

    entry = {"img":None,"bounds":None,"center":[49.5,2.4],
             "vmin":0,"vmax":1,"classes":[],"pts":None,"imp":{},"met":{},
             "modelable": depth != "100-200"}

    # ── RF Mean map (only for modelable depths) ──────────────────
    if entry["modelable"]:
        tif_path = os.path.join(depth_dir, f"carte_RF_moyenne_{field}.tif")
        if os.path.exists(tif_path):
            ...
            vmin = GLOBAL_VMIN
            vmax = GLOBAL_VMAX
            entry["vmin"] = round(vmin,1)
            entry["vmax"] = round(vmax,1)
            entry["classes"] = GLOBAL_CLASSES

            # Apply colormap → PNG base64 (normalisation globale fixe)
            norm  = np.clip((data-vmin)/max(vmax-vmin,1e-9),0,1)
            rgba  = cmap(norm)
            rgba[...,3] = np.where(np.isnan(data),0,0.85)
            ...
            # Subsample pixel values for distribution boxplot (max 1500 pts)
            ...
            print(f"  Map not found: {tif_path}")

    # ── Sample points (clipped to Oise) ──────────────────────────
    shp_path = os.path.join(fo_depth, f"points_apprentissage_{field_nv}.shp")
    if os.path.exists(shp_path):
        try:
            geojson, n = shp_to_geojson(shp_path)
            entry["pts"] = geojson
            print(f"  Sample points OK — {n} pts (after clipping to Oise)")
        except Exception as e:
            print(f"  Sample points error: {e}")
    else:
        print(f"  Sample points not found: {shp_path}")

    # ── MDI importance (modelable only) ──────────────────────────
    if entry["modelable"]:
        imp_path = os.path.join(depth_dir,
                                f"importance_MDI_50iter_redistrib_{field}.xlsx")
    ...
    # ── Validation metrics (modelable only) ──────────────────────
    if entry["modelable"]:
        met_path = os.path.join(depth_dir,
                                f"metriques_iterations_{field}.xlsx")
...
<title>DSM — Clay Content · Oise</title>
...
#header-left h1{{
  font-size:0.92rem;font-weight:700;color:#e2e8f0;letter-spacing:0.3px;
}}
#header-left p{{
  font-size:0.68rem;color:rgba(255,255,255,0.38);margin-top:1px;
}}
...
/* Points d'échantillonnage — ligne toggleable */
.leg-pts{{
  display:flex;align-items:center;gap:9px;
  padding:5px 7px;
  margin:0 -7px;
...
.leg-pts.pts-off .pt-icon{{opacity:0.22;}}
...
/* Icône mini-dégradé pour le raster */
.raster-icon{{
...
/* ── Contrôles Leaflet — thème sombre ────────────────────────── */
...
/* ── Sidebar statistiques — côté GAUCHE ──────────────────────── */
#sidebar{{
  position:fixed;top:54px;left:0;bottom:0;z-index:1080;
  width:375px;
  display:flex;flex-direction:row;
  /* fermée : tout masqué sauf les 28px du bouton hamburger */
...
.info-btn{{
  ...
  justify-content:center;flex-shrink:0;transition:color .15s,border-color .15s;
  padding:0;
}}
.info-btn:hover{{color:#93c5fd;border-color:#93c5fd;}}
.info-btn.active{{color:#38bdf8;border-color:#38bdf8;}}
#var-info-panel{{
  display:none;overflow-y:auto;max-height:260px;
...
<div id="header">
  <div id="header-left">
    <h1>Digital Soil Mapping — Clay Content (g/kg)</h1>
...
<!-- ── Légende ─────────────────────────────────────────────────── -->
<div id="legend-panel">
  <div class="leg-title">Clay content (g/kg)</div>
  ...
  <div class="leg-pts" id="raster-toggle" onclick="toggleRaster()">
    ...
    <span class="pt-label" id="raster-label">Clay map</span>
...
// Fond sombre CARTO Dark Matter (pas de clé API requise)
L.tileLayer(
...
// ── Contour département Oise (couche permanente) ──────────────
...
// ── Utilitaires ───────────────────────────────────────────────
function ptColor(value, classes) {{
  if (value === null || classes.length === 0) return 'rgba(200,200,200,0.7)';
...
function updateLegend(classes) {{
  ...
  if (!currentClasses.length) {{ el.innerHTML = ''; return; }}
  el.innerHTML = currentClasses.map(c =>
    `<div class="leg-item">
       <div class="leg-swatch" style="background:${{c.color}}"></div>
       <span>${{c.label}}</span>
     </div>`
  ).join('');
  // Mise à jour de l'icône dégradé raster
  if (currentClasses.length >= 2) {{
    const first = currentClasses[0].color;
    const last  = currentClasses[currentClasses.length-1].color;
    document.getElementById('raster-icon').style.background =
      `linear-gradient(to right,${{first}},${{last}})`;
  }}
}}

// ── Chargement d'une profondeur ───────────────────────────────
function showDepth(d, btn) {{
  document.querySelectorAll('.dtab').forEach(t => t.classList.remove('active'));
  btn.classList.add('active');
  currentDepth = d;

  // Reset var info panel on depth change
  if (varInfoVisible) {{
  ...
  // Supprimer couches raster et points de la profondeur précédente
  ...
  // Mise à jour du label de la légende raster avec la profondeur courante
  const depthLabels = {{"0-5":"0–5","5-15":"5–15","15-30":"15–30","30-60":"30–60","60-100":"60–100","100-200":"100–200"}};
  document.getElementById('raster-label').textContent = `Clay map (${{depthLabels[d]}} cm)`;

  // Overlay raster RF
  if (info.img && info.bounds) {{
    rfLayer = L.imageOverlay(
      'data:image/png;base64,' + info.img,
      info.bounds,
  ...
  // Remonter le contour département au-dessus des autres couches
  if (deptLayer) deptLayer.bringToFront();

  // Rafraîchir les graphiques si la sidebar est ouverte
...
function toggleRaster() {{
  ...
  if (rfLayer) {{
    if (rfOn) rfLayer.addTo(map);
    else      map.removeLayer(rfLayer);
  }}
}}

// ── Toggle points d'échantillonnage ──────────────────────────
function togglePoints() {{
  ptsOn = !ptsOn;
  document.getElementById('pts-toggle').classList.toggle('pts-off', !ptsOn);
  if (ptLayer) {{
    if (ptsOn) ptLayer.addTo(map);
    else       map.removeLayer(ptLayer);
  }}
}}

// ── Toggle contour département ────────────────────────────────
function toggleDept() {{
  deptOn = !deptOn;
  document.getElementById('dept-toggle').classList.toggle('pts-off', !deptOn);
  if (deptLayer) {{
    if (deptOn) deptLayer.addTo(map);
    else        map.removeLayer(deptLayer);
  }}
}}
...
function renderCharts(d) {{
  ...
  // ── MDI importance — boxplots horizontaux ─────────────────
  const imp     = info.imp;
  const vars    = Object.keys(imp);           // triés médiane desc (Python)
  const varsAsc = [...vars].reverse();         // ascendant → plus important en haut du graphe
  ...
  Plotly.react(impEl,
    varsAsc.map(v => {{
      ...
      return {{
      x          : imp[v],
      name       : lbl.length > 22 ? lbl.slice(0,20)+'…' : lbl,
      type       : 'box',
      orientation: 'h',
      boxpoints  : 'outliers',
    ...
    {{
      ...
      xaxis:{{
        title     : {{ text:'Importance (%)', font:{{ color:'rgba(255,255,255,0.36)', size:10 }} }},
      ...
      yaxis:{{
        tickfont  : {{ size:9, color:'rgba(255,255,255,0.62)' }},
        automargin: true,
        color     : 'rgba(255,255,255,0.32)',
      }},
      hovermode : false,
  ...
  // ── Métriques R² et RMSE — box + tous les points ─────────
  const met      = info.met;
  const r2s      = met.r2;
  ...
  Plotly.react(metEl,
    [
      {{
        ...
        line     : {{ color:'#6ee7b7', width:1.8 }},
        fillcolor: 'rgba(52,211,153,0.11)',
        showlegend:false,
      }},
      {{
        y        : rmses,
        ...
        line     : {{ color:'#fdba74', width:1.8 }},
        fillcolor: 'rgba(251,146,60,0.11)',
        showlegend:false,
      }},
    ],
    {{
      height        : 260,
      margin        : {{ l:38, r:38, t:66, b:30 }},
      grid          : {{ rows:1, columns:2, pattern:'independent' }},
      paper_bgcolor : 'rgba(0,0,0,0)',
      plot_bgcolor  : 'rgba(255,255,255,0.015)',
      xaxis:{{
        domain:[0,0.43],
        title :{{ text:'R²', font:{{ color:'#6ee7b7', size:11 }} }},
        tickfont:{{ size:9, color:'rgba(255,255,255,0.42)' }},
        color:'rgba(255,255,255,0.22)',gridcolor:'rgba(0,0,0,0)',
      }},
      xaxis2:{{
        domain:[0.57,1],
        title :{{ text:'RMSE (g/kg)', font:{{ color:'#fdba74', size:11 }} }},
        tickfont:{{ size:9, color:'rgba(255,255,255,0.42)' }},
        color:'rgba(255,255,255,0.22)',gridcolor:'rgba(0,0,0,0)',
      }},
      yaxis:{{
      ...
      shapes:[
        {{ type:'line', xref:'x', yref:'y', x0:-0.45, x1:0.45, y0:0.5, y1:0.5,
      ...
      annotations:[
        ...
        {{ xref:'paper', yref:'paper', x:0.785, y:1.01,
           ...
           showarrow:false,
           font:{{ color:'#fdba74', size:9.5 }},
           xanchor:'center', yanchor:'bottom', align:'center' }},
        {{ xref:'paper', yref:'y', x:0.43, y:0.5,
           text:'0.5', showarrow:false,
           font:{{ color:'rgba(239,68,68,0.65)', size:9 }},
  ...
  if (rv.length === 0) {{
    ...
    Plotly.react(rastEl,
      ...
      {{
        ...
        xaxis:{{
          title:{{text:'Clay (g/kg)',font:{{color:'rgba(255,255,255,0.36)',size:10}}}},
        ...
        annotations:[{{
          ...
          showarrow:false,font:{{color:'#fbbf24',size:9}},
