"""
Generates index.html for DSM CEC (Cation Exchange Capacity) — Oise.
Fixed 5-class CEC classification (meq/100g), dark map, collapsible stats sidebar.
100-200 cm: not modelled (sample points only).
"""
...
FO_BASE    = os.path.join(SCRIPT_DIR, "..", "Final_outputs", "Final_outputs", "CEC")
...
DEPTH_FOLDER = {"0-5":"0-5","5-15":"5-15","15-30":"15-30",
                "30-60":"30-60","60-100":"60-100","100-200":"100-200"}
NOT_MODELLED = {"100-200"}

# ── Fixed CEC classification (5 classes, meq/100g) ────────────────
CEC_LABELS = ['≤ 5', '5 – 10', '10 – 15', '15 – 20', '≥ 20']

# Sequential gradient: orange → red (distinct from all other maps)
CMAP_CEC = plt.get_cmap('OrRd')
NORM_CEC  = mcolors.Normalize(vmin=0, vmax=20, clip=True)

_mids = [2.5, 7.5, 12.5, 17.5, 20.0]
def _cec_hex(v):
    return mcolors.to_hex(CMAP_CEC(NORM_CEC(v))[:3])

FIXED_CLASSES = [
    {"min": -999, "max": 5,    "color": _cec_hex(_mids[0]), "label": CEC_LABELS[0]},
    {"min": 5,    "max": 10,   "color": _cec_hex(_mids[1]), "label": CEC_LABELS[1]},
    {"min": 10,   "max": 15,   "color": _cec_hex(_mids[2]), "label": CEC_LABELS[2]},
    {"min": 15,   "max": 20,   "color": _cec_hex(_mids[3]), "label": CEC_LABELS[3]},
    {"min": 20,   "max": 9999, "color": _cec_hex(_mids[4]), "label": CEC_LABELS[4]},
]
...
for depth in DEPTHS:
    ...
    if entry["modelable"]:
        tif_path = find_file(depth_dir,
                             f"carte_RF_moyenne_CEC_meq_100g_{suffix}.tif")
        if tif_path:
            ...
            rgba = CMAP_CEC(NORM_CEC(np.where(np.isnan(data), 0, data)))
    ...
    # ── Sample points ─────────────────────────────────────────────
    shp_path = os.path.join(depth_dir, f"points_apprentissage_CEC_nv_meq_100g_{suffix}.shp")
    ...
    imp_path = find_file(depth_dir,
                         f"importance_MDI_50iter_redistrib_CEC_meq_100g_{suffix}.xlsx")
    if imp_path:
    ...
    met_path = find_file(depth_dir,
                         f"metriques_iterations_CEC_meq_100g_{suffix}.xlsx")
    if met_path:
...
<div id="header">
  <div id="header-left">
    <h1>Digital Soil Mapping — Cation Exchange Capacity</h1>
...
<div id="legend-panel">
  <div class="leg-title">CEC (meq/100g)</div>
  ...
  <div class="leg-pts" id="raster-toggle" onclick="toggleRaster()">
    ...
    <span class="pt-label" id="raster-label">CEC map</span>
...
function showDepth(d, btn) {{
  ...
  document.getElementById('raster-label').textContent = `CEC map (${{depthLabels[d]}} cm)`;
  ...
  if (info.pts && info.pts.features.length > 0) {{
    ptLayer = L.geoJSON(info.pts, {{
      ...
      onEachFeature:(f,layer) => {{
        ...
        layer.bindTooltip(
          v !== null ? `<b>CEC:</b> ${{v.toFixed(2)}} meq/100g` : '<i>No value</i>',
...
function renderCharts(d) {{
  ...
  Plotly.react(metEl,
    ...
    {{
      ...
      xaxis2:{{domain:[0.57,1],title:{{text:'RMSE (meq/100g)',font:{{color:'#fdba74',size:11}}}},
        tickfont:{{size:9,color:'rgba(255,255,255,0.42)'}},color:'rgba(255,255,255,0.22)',gridcolor:'rgba(0,0,0,0)'}},
      ...
      annotations:[
        ...
        {{xref:'paper',yref:'paper',x:0.785,y:1.01,
          text:`Max: ${{maxRmse}}<br>Mean: ${{meanRmse}}<br>Min: ${{minRmse}} meq/100g`,
  ...
  if (rv.length === 0) {{
    ...
    Plotly.react(rastEl,
      ...
      {{
        ...
        paper_bgcolor:'rgba(0,0,0,0)', plot_bgcolor:'rgba(255,255,255,0.015)',
        xaxis:{{
          title:{{text:'CEC (meq/100g)',font:{{color:'rgba(255,255,255,0.36)',size:10}}}},
          color:'rgba(255,255,255,0.32)',gridcolor:'rgba(255,255,255,0.06)',
        ...
        annotations:[{{
          ...
          text:`Max: ${{maxV}} · Mean: ${{meanV}} · Min: ${{minV}} meq/100g`,
